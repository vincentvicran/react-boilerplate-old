...
export const reactNodeIsOfType = <
  P extends object,
  T extends {(props: P): ReactElement | null | undefined}
>(
  node: ReactNode,
  type: T
): node is any => isObject<ReactElement>(node) && node.type === type
...
export const getNewChild = (children: any | ReactNode, value: string) => {
  return Children.toArray(children).filter(
    (child: any) => child.props.id === value
  )
}
